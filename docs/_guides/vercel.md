...
| 2026-08-08 | 1.2.1 | draft | Add Code Pack for 3.5 Vercel BotID (`hth-vercel-3.05`, sdk type): `withBotId()` next.config wrap, `initBotId()` client route declarations, and `checkBotId()` server-side handler gate, all fetch-verified against vercel.com/docs/botid/get-started; wired the 3.5 pack include | Claude Code (Fable 5) |
