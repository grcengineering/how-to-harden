...
**Option 1: Okta API**
```bash
# Step 1: List all authentication policies and identify the default
curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/policies?type=ACCESS_POLICY" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json" \
  | jq '.[] | {id: .id, name: .name, system: .system, status: .status}'

# Step 2: For each policy, list assigned applications
# Replace POLICY_ID with each policy ID from Step 1
curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/policies/${POLICY_ID}/app" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json" \
  | jq '.[] | {id: .id, name: .name, label: .label}'

# Step 3: Full audit script -- find all apps on the default policy
# First, find the default policy ID (system=true for the default)
DEFAULT_POLICY_ID=$(curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/policies?type=ACCESS_POLICY" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  | jq -r '.[] | select(.system == true and .name == "Default Policy") | .id')

echo "Default Policy ID: ${DEFAULT_POLICY_ID}"

# List apps assigned to the default policy
curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/policies/${DEFAULT_POLICY_ID}/app" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  | jq '.[] | {id: .id, label: .label, status: .status}'

# Step 4: Reassign an application to a custom policy
# Replace TARGET_POLICY_ID with your MFA-enforcing policy ID
# Replace APP_ID with the application to reassign
curl -s -X PUT "https://${OKTA_DOMAIN}/api/v1/apps/${APP_ID}/policies/${TARGET_POLICY_ID}" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json"
```
...
**Option 2: Terraform**
```hcl
# terraform/okta/default-policy-audit.tf

# Data source to reference the default authentication policy
data "okta_policy" "default_access" {
  name = "Default Policy"
  type = "ACCESS_POLICY"
}

# Create a custom catch-all policy for applications
...
# Deny rule as catch-all (lowest priority in custom policy)
...
# Assign each application to the custom policy instead of default
# Repeat this block for every application in your tenant
resource "okta_app_signon_policy_rule" "app_assignment" {
  # Managed via okta_app_oauth, okta_app_saml, etc.
  # Each app resource supports: authentication_policy = okta_app_signon_policy.mfa_required.id
}
```
...
**Option 1: Okta API**
```bash
# Step 1: List current password policy settings including recovery configuration
curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/policies?type=PASSWORD" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json" \
  | jq '.[] | {id: .id, name: .name, settings: .settings.recovery}'

# Step 2: Update password policy to restrict recovery methods
# Replace POLICY_ID with your password policy ID
curl -s -X PUT "https://${OKTA_DOMAIN}/api/v1/policies/${POLICY_ID}" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
...
# Step 3: Deactivate the Security Question authenticator
# First, find the authenticator ID
SECURITY_QUESTION_ID=$(curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/authenticators" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  | jq -r '.[] | select(.key == "security_question") | .id')

# Deactivate it
curl -s -X POST "https://${OKTA_DOMAIN}/api/v1/authenticators/${SECURITY_QUESTION_ID}/lifecycle/deactivate" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}"

# Step 4: Deactivate Phone (SMS/Voice) authenticator for recovery
PHONE_ID=$(curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/authenticators" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  | jq -r '.[] | select(.key == "phone_number") | .id')

# Update phone authenticator to remove recovery usage
curl -s -X PUT "https://${OKTA_DOMAIN}/api/v1/authenticators/${PHONE_ID}" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Phone",
...
**Option 2: Terraform**
```hcl
# terraform/okta/harden-recovery.tf
...
# Configure phone authenticator -- remove recovery usage
...
**Option 1: Okta API**
```bash
# Step 1: Get current org settings
curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/org/settings" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json" \
  | jq '{endUserNotifications: .endUserNotifications, suspiciousActivityReporting: .suspiciousActivityReporting}'

# Step 2: Enable all end-user notifications
curl -s -X PUT "https://${OKTA_DOMAIN}/api/v1/org/settings" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
...
# Step 3: Enable Suspicious Activity Reporting
curl -s -X POST "https://${OKTA_DOMAIN}/api/v1/org/privacy/suspicious-activity-reporting" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
    "enabled": true
...
curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/org/settings" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  | jq '.endUserNotifications'

curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/org/privacy/suspicious-activity-reporting" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}"
```

**Option 2: Terraform**
```hcl
# terraform/okta/end-user-notifications.tf

# Note: As of the Okta Terraform provider, org-level notification
# settings may require the okta_org_configuration resource or
# direct API provisioning via null_resource.
...
resource "okta_org_configuration" "notifications" {
  ...
  # Use a provisioner or the Okta API provider for full control.
}
...
**Option 1: Okta API**
```bash
# List all network zones to find the Enhanced Dynamic Zone
curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/zones" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  | jq '.[] | {id: .id, name: .name, type: .type, status: .status, usage: .usage}'

# Activate the DefaultEnhancedDynamicZone as a blocklist
ZONE_ID=$(curl -s -X GET "https://${OKTA_DOMAIN}/api/v1/zones" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  | jq -r '.[] | select(.name == "DefaultEnhancedDynamicZone") | .id')

curl -X PUT "https://${OKTA_DOMAIN}/api/v1/zones/${ZONE_ID}" \
  -H "Authorization: SSWS ${OKTA_API_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
    "type": "DYNAMIC_V2",
...
**Option 2: Terraform**
```hcl
# terraform/okta/anonymizer-blocking.tf
...
resource "okta_network_zone" "block_countries" {
  ...
  status   = "ACTIVE"
  usage    = "BLOCKLIST"
  dynamic_locations = ["CN", "RU", "KP", "IR"]  # Adjust per policy
}
```
