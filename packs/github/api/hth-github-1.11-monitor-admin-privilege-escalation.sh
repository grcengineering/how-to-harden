...
#
# Requires GitHub Enterprise Cloud (the organization audit-log API) and a token
# that can read the audit log.
...
gh api "/orgs/${GITHUB_ORG}/audit-log?phrase=action:org.update_member_role" \
