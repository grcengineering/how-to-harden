...
#   OKTA_API_TOKEN    -- SSWS API token with admin privileges
...
# ---------------------------------------------------------------------------
okta_get() {
  curl -sf -X GET "${OKTA_BASE}$1" \
    -H "${AUTH_HEADER}" \
    -H "Content-Type: application/json"
}

okta_post() {
  curl -sf -X POST "${OKTA_BASE}$1" \
    -H "${AUTH_HEADER}" \
    -H "Content-Type: application/json" \
    -d "$2"
}
