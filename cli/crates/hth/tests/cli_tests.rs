...
fn test_validate_all_packs() {
    ...
    let text = full_output(&output);
    assert!(
        text.contains("65 controls validated"),
        "expected '65 controls validated' in output, got: {text}"
    );
    assert!(
...
fn test_scan_dry_run_shows_control_count() {
    ...
    let text = full_output(&output);
    assert!(
        text.contains("Scanning 31 controls"),
        "dry-run should report 'Scanning 31 controls' for github, got: {text}"
    );
}
...
fn test_packs_resolution_from_non_repo_dir() {
    ...
    assert!(
        text.contains("65 controls validated"),
        "should validate all 65 controls from non-repo dir, got: {text}"
    );
}
...
fn test_env_packs_dir_variable() {
    ...
    assert!(
        text.contains("65 controls validated"),
        "should validate all 65 controls via env var, got: {text}"
    );
}
