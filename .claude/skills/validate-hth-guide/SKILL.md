...
4. **Badge exactly the VERIFIED-LIVE controls.** One line per qualifying control, on its own line **between** the `### N.N` heading and the `**Profile Level:**` line:

   `{% include status-badge.html status="ai-validated" evidence="{what was actually exercised}" date="{YYYY-MM-DD}" %}`

   The include is `status-badge.html` and it takes an explicit `status` — the old `ai-validated.html` is gone, and so is its implicit single meaning. `status` accepts any of the six statuses; this run writes exactly one of them. Two badges on one control (a machine and a person both exercised it) go on consecutive lines in the same slot: they add, they do not replace.

   `evidence` is required in practice — it is what makes the badge falsifiable, so write what was run ("console path re-read on the live org; read-only API audit pack executed against a live tenant"), never a restatement of the control title. **Placement is load-bearing and every wrong slot fails silently**: inside the heading it rewrites the kramdown anchor that 2,300+ in-guide links and every pack `guide_url` depend on; in the same paragraph as `**Profile Level:**` the cheat-sheet parser stops seeing the level and the control vanishes from the sheet; under `#### Description` the badge text is captured as the description cell. The include's own comment block is the reference — read it before stamping the first badge.
5. **Forbidden badges — no exceptions, and each wrong for its own reason:**
...
6. **A partial promotion is the normal outcome, and it is honest.** A guide gaining `ai-validated` with 11 of 22 controls badged says precisely that: eleven requirements were exercised live, eleven were not, and the reader can see which is which. Do **not** badge the remainder to make the page look finished, and do not withhold the page-level status because coverage is partial. The page status is the sum of the badges; that is why they render the same mark.
7. **Demote on contradiction.** `ai-validated` is a claim about the world, so it expires when the world moves. A later run (or an `update-hth-guide` currency pass) that finds a console path moved or a setting removed **removes** `ai-validated` from the set — back to `["ai-drafted"]` — and strips the badges that finding invalidated. Leaving a stale status standing is worse than never having added it.
...
grep -c 'include status-badge.html' docs/_guides/{slug}.md  # == VERIFIED-LIVE count in the ledger
...
- **The badge is the status, retail.** `docs/_includes/status-badge.html` renders the same mark as the page banner on purpose, so a badge on a control nobody exercised does not read as a small slip — it reads to the viewer as the page-level claim, applied to that requirement. Stamp from the ledger, one badge per `VERIFIED-LIVE` row, never from memory of what the run "basically covered".
