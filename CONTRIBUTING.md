...
HTH uses **Extended SemVer with a derived maturity qualifier**, aligned with [CIS Benchmarks](https://www.cisecurity.org/cis-benchmarks) versioning practices. See [VERSIONS.md](VERSIONS.md) for full documentation.
...
**What each status asserts -- and, just as importantly, what it does not -- is defined once in [VERSIONS.md](VERSIONS.md#maturity-statuses--the-matrix).** Don't restate it here or in a guide; link to it.
...
| `ai-validated` | Only a [`validate-hth-guide`](.claude/skills/validate-hth-guide/SKILL.md) run, as its close-out step. **Never hand-typed** — the value is a claim that a live tenant was exercised, so typing it without that run is a false claim, not a formatting choice. |
