...
**This section is the single definition of the maturity vocabulary.** README.md, CONTRIBUTING.md, `docs/contributing.md`, `templates/vendor-guide-template.md`, the authoring skills in `.claude/skills/`, `docs/_includes/status-set.html`, `docs/_includes/status-mark.html`, `docs/_layouts/guide.html`, `docs/index.html`, and `scripts/validate-guides.sh` all point here instead of carrying their own copy. If a status's meaning changes, it changes here first and the rest follow.
...
Last updated: 2026-09-24 — 130 guides. Every one is `ai-drafted`; two (Buildkite, Ona) additionally hold `ai-validated`. **No guide holds any `ni-*` status** — the whole natural-intelligence half of the matrix is currently empty.
